...
function Yawscraper(options, callback) {
  if (!(this instanceof Yawscraper))
    return new Yawscraper(options, callback);
  ...
  if (!this.options.allOcurrencies)
    this.options.ocurrencies = true;
  if (!this.options.forceMatchAll)
    this.options.forceMatchAll = true;
